      ...
      if (id && collection_status && payment_id && status && preference_id) {
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/payment/failure?id=${id}&collection_status=${collection_status}&payment_id=${payment_id}&status=${status}&preference_id=${preference_id}`,
            {
              method: "GET",
            }
          );
